...
class AIAgentDashboard:
    ...
    def _get_data(self) -> Optional[pd.DataFrame]:
        ...
        try:
            if self.data_source == "Upload CSV":
                ...
                if sheet_url:
                    return self.data_handler.connect_google_sheet(
                        sheet_url,
    ...
    def _process_emails(self, df: pd.DataFrame, batch_id: str):
        ...
        try:
            ...
            st.write("Processing email templates with LLM...")
            ...
            # Process each row's content with LLM before sending
            processed_rows = []
            for _, row in df.iterrows():
                ...
                try:
                    ...
                    st.error(f"Error processing content with LLM: {str(llm_error)}")
            ...
            # Create batch
            batch_id = self.email_handler.create_batch(df, template_config)
            ...
            if self.schedule_type != "Send Immediately":
                ...
                schedule_config = {
                    'type': 'batch' if self.schedule_type == "Batch Schedule" else 'single',
                    'start_time': schedule_datetime.isoformat(),
                    'batch_size': getattr(self, 'batch_size', None),
                    'interval_hours': getattr(self, 'batch_interval', None)
                }
            
            # Process the batch
            success = self.email_handler.process_batch(batch_id, schedule_config)
            
            if success:
                st.success("Email campaign started successfully!")
            else:
                st.error("Failed to start email campaign.")
